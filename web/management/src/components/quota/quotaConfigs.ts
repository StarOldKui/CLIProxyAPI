...
export interface QuotaConfig<TState, TData> {
  ...
  getCachedState?: (file: AuthFileItem, t: TFunction) => TState | null;
  getCachedStateKey?: (file: AuthFileItem) => string;
  cardClassName: string;
  controlsClassName: string;
  controlClassName: string;
  gridClassName: string;
  sortFiles?: (files: AuthFileItem[], quota: Record<string, TState>) => AuthFileItem[];
  groupFiles?: (
    files: AuthFileItem[],
    quota: Record<string, TState>,
    t: TFunction
  ) => Array<{ key: string; label: string; files: AuthFileItem[] }>;
  renderQuotaItems: (quota: TState, t: TFunction, helpers: QuotaRenderHelpers) => ReactNode;
}

const CODEX_PLAN_ORDER = ['pro', 'plus', 'free', 'unknown'];

const resolveCodexGroupPlan = (
  file: AuthFileItem,
...
const compareByName = (a: AuthFileItem, b: AuthFileItem): number =>
  a.name.localeCompare(b.name, undefined, { sensitivity: 'base' });

const sortCodexFiles = (
  files: AuthFileItem[],
  quota: Record<string, CodexQuotaState>
): AuthFileItem[] =>
  [...files].sort((a, b) => {
    const planDiff =
      CODEX_PLAN_ORDER.indexOf(resolveCodexGroupPlan(a, quota[a.name])) -
      CODEX_PLAN_ORDER.indexOf(resolveCodexGroupPlan(b, quota[b.name]));
    return planDiff || compareByName(a, b);
  });

const groupCodexFiles = (
  files: AuthFileItem[],
  quota: Record<string, CodexQuotaState>,
  t: TFunction
): Array<{ key: string; label: string; files: AuthFileItem[] }> => {
  const labels: Record<string, string> = {
    ...
    plus: 'Plus',
    free: 'Free',
    unknown: t('codex_quota.plan_unknown', { defaultValue: 'Unknown' }),
  };
  return CODEX_PLAN_ORDER.map((plan) => ({
    key: plan,
    label: labels[plan],
    files: files.filter((file) => resolveCodexGroupPlan(file, quota[file.name]) === plan),
...
const buildCodexQuotaDataFromSnapshot = (
  ...
  const subscriptionActiveStart =
    snapshot.subscription_active_start ?? snapshot.subscriptionActiveStart ?? null;
  const subscriptionActiveUntil =
    snapshot.subscription_active_until ?? snapshot.subscriptionActiveUntil ?? null;
  ...
  return {
    planType: planTypeFromUsage ?? normalizePlanType(snapshot.plan_type) ?? planTypeFromFile,
    windows,
...
const fetchCodexQuota = async (
  ...
  const response = await quotaApi.refreshCodex([file.name]);
  const snapshot = response.results?.[file.name];
  if (!snapshot) {
    throw new Error(t('codex_quota.empty_windows'));
  }
  return buildCodexQuotaDataFromSnapshot(file, snapshot, t);
};
...
const parseCodexSubscriptionDate = (value?: string | number | null): Date | null => {
...
const formatCodexSubscriptionRemaining = (date: Date, t: TFunction): string => {
  const remainingMs = date.getTime() - Date.now();
  if (remainingMs <= 0) {
    return t('codex_quota.subscription_expired');
  }
  const days = Math.ceil(remainingMs / 86_400_000);
  if (days >= 1) {
    return t('codex_quota.subscription_days', { count: days });
  }
  const hours = Math.ceil(remainingMs / 3_600_000);
  return t('codex_quota.subscription_hours', { count: hours });
};
...
const renderCodexItems = (
  ...
  const subscriptionDate = parseCodexSubscriptionDate(quota.subscriptionActiveUntil);
  ...
  if (subscriptionDate) {
    nodes.push(
      h(
        ...
        h('span', { className: styleMap.codexPlanLabel }, t('codex_quota.subscription_label')),
        h(
          'span',
          { className: styleMap.codexPlanValue },
          `${formatCodexSubscriptionRemaining(subscriptionDate, t)} ${subscriptionDate.toLocaleString()}`
        )
      )
    );
  }
