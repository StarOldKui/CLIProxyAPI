...
export const isUsageLimitReachedMessage = (message: string): boolean => {
  if (!message) return false;
  try {
    const parsed = JSON.parse(message) as { error?: { type?: unknown } };
...
export const isUsageLimitReachedAuthFile = (file: AuthFileItem): boolean => {
  ...
  return isUsageLimitReachedMessage(message);
};

export const hasAuthFileProblemStatus = (file: AuthFileItem): boolean =>
  hasAuthFileStatusMessage(file) && !isUsageLimitReachedAuthFile(file);
